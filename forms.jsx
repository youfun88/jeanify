        ...
        <div className="container-tight">
          ...
          <div style={{ background:'var(--bg-elev)', padding: 56, border:'1px solid var(--line)' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', marginTop: 40, paddingTop: 32, borderTop:'1px solid var(--line)' }}>
              <button className="btn btn-ghost" disabled={step===1} onClick={()=>setStep(s=>Math.max(1, s-1))} style={{ opacity: step===1 ? 0.4 : 1 }}>← Back</button>
              {step < 3 ? (
                <button className="btn btn-primary arrow-right" onClick={()=>setStep(s=>s+1)}>Continue</button>
              ) : (
                <button className="btn btn-primary arrow-right" onClick={()=>{ alert(lang==="en"?"Thank you. Jean will follow up within 48 hours.":"已收到，Jean 將在 48 小時內聯絡您。"); go("home"); }}>Submit Request</button>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="contact-layout">
            <div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap: 32 }}>
                {[
                  ["Phone", D4.agent.phone, lang==="en"?"Direct line, 9am – 7pm PT":"直線，太平洋時間 9 - 19 時"],
                  ["Email", D4.agent.email, lang==="en"?"Replies within one business day":"一個工作日內回覆"],
                  ["Office", "9888 Carroll Centre Rd, Ste 200", "San Diego, California 92126"],
                  ["Languages", "English · 中文 (Mandarin) · 台語 (Taiwanese)", lang==="en"?"All documents reviewable in either language":"所有檔案支援中英雙語審閱"],
                ].map(([k, v, d]) => (
                  <div key={k} style={{ paddingBottom: 24, borderBottom:'1px solid var(--line)' }}>
                    <div className="eyebrow" style={{ marginBottom: 12 }}>{k}</div>
                    <div style={{ fontFamily:'var(--font-display)', fontSize: 26, color:'var(--ink)' }}>{v}</div>
    ...
    <div className="page-fade">
      <header className="page-head">
        <div className="container">
          ...
          <p className="lede">{lang==="en"?"Property tours, neighborhood walkthroughs and stories from clients and fellow agents — straight from Jean's YouTube channel.":"房源實地走訪、社群導覽，以及來自客戶與同業經紀人的真實分享 —— 全部來自 Jean 的 YouTube 頻道。"}</p>
